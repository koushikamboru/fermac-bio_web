    ...
    <section className="py-12 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {directors.concat(directors.slice(0,2)).map((d, idx) => (
            <div key={`${d.name}-${idx}`} className="bg-cyan-50 rounded-xl border border-cyan-100 p-6">
              <div className="flex items-center justify-center w-16 h-16 rounded-full bg-gray-200 text-gray-400 text-2xl mb-4 mx-auto">🖼️</div>
              <h3 className="text-center font-semibold text-teal-900">{d.name}</h3>
              <p className="text-center text-sm text-gray-500 italic mb-4">{d.title}</p>

              <div className="text-sm text-gray-700 leading-relaxed mb-4">
                <span className="font-semibold text-cyan-600">Profile:</span>
                <div className="mt-1 text-sm text-gray-700">{d.profile}</div>
              </div>

              <div>
                <h4 className="font-semibold text-cyan-600 mb-2 text-sm">Expertise:</h4>
